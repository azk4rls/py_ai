...
# PENTING: KOREKSI briefing_user INI
briefing_user = """
...
def new_chat():
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            # PERBAIKAN 1: HAPUS PENYIMPANAN BRIEFING KE DATABASE DI new_chat
            # Briefing akan ditambahkan saat panggilan ke AI di endpoint /ask
...
def ask_ai():
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            # PERBAIKAN 2: TAMBAHKAN BRIEFING KE HISTORY_FOR_AI DI SINI
            # Briefing ini hanya untuk model AI, tidak disimpan di DB sebagai chat history UI
            history_for_ai = [
                {"role": 'user', "parts": [briefing_user]},
                {"role": 'model', "parts": [briefing_model]}
            ]
            # Tambahkan history chat sebenarnya dari database
            ...
            cur.execute('INSERT INTO messages (conversation_id, role, content) VALUES (%s, %s, %s)', (conversation_id, 'assistant', ai_answer)) # Simpan AI as 'assistant'
            ...
            # PERBAIKAN 3: Sesuaikan logika judul percakapan
            # Karena briefing user tidak disimpan, pesan user pertama adalah count 1
...
def get_conversation(conversation_id):
    ...
    try:
        conn = get_db_connection()
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            # PERBAIKAN 4: Pastikan hanya pesan user dan assistant yang ditampilkan di UI
            # Ini sudah ada, tapi penting untuk diverifikasi
